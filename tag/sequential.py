...
######################################################################
#{ Demo code (should be turned into doctests)
######################################################################
...
def demo():
    ...
    t1 = tag.UnigramTagger.train(train_data, cutoff=1, backoff=t0,
                                    verbose=True)

    t2 = tag.AffixTagger.train(train_data, -3, 2, cutoff=2, backoff=t0,
                                  verbose=True)

    t3 = tag.RegexpTagger([(r'.*ed', 'VBD')], backoff=t0)

    t4 = tag.UnigramTagger({'the': 'DT'}, backoff=t0)
