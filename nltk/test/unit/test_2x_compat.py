...
def setup_module(module):
    ...
    if PY3:
        raise SkipTest(
            "test_2x_compat is for testing nltk.compat under Python 2.x")
...
class TestTextTransliteration(unittest.TestCase):
    ...
    def test_repr(self):
        self.assertEqual(
            repr(self.txt), br"<Text: S\xe3o Tom\xe9 and Pr\xedncipe...>")
...
class TestFraction(unittest.TestCase):
    def test_unnoramlize_fraction(self):
        ...
        assert float(one_two) == float(
            six_twelve) == float(six_twelve_original)

        # Checks that the fraction does get normalized, even when
        # _normalize == False when numerator is using native
        # fractions.Fraction.from_float
        assert NLTKFraction(
            3.142, _normalize=False) == NativePythonFraction(3.142)
