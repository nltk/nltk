...
class TestEditDistanceWithHigherSubCost(unittest.TestCase):
    ...
    def test_with_transpositions(self):
        self.assertEqual(
            edit_distance(
                self.left, self.right, substitution_cost=2, transpositions=True
            ),
            4,
        )
        self.assertEqual(
            edit_distance(
                self.right, self.left, substitution_cost=2, transpositions=True
            ),
            4,
        )
    ...
    def test_without_transpositions(self):
        self.assertEqual(
            edit_distance(
                self.left, self.right, substitution_cost=2, transpositions=False
            ),
            5,
        )
        self.assertEqual(
            edit_distance(
                self.right, self.left, substitution_cost=2, transpositions=False
            ),
            5,
        )
