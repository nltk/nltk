...
class WordnNetDemo(unittest.TestCase):
    ...
    def test_hyperhyponyms(self):
        ...
        self.assertEqual(
            sorted(S("meal.n.1").hyponyms()[:5]), first_five_meal_hypo
        )
        self.assertEqual(
            S("Austen.n.1").instance_hypernyms(), [S("writer.n.01")]
        )
    ...
    def test_meronyms_holonyms(self):
        ...
        self.assertEqual(
            S("faculty.n.2").member_meronyms(), [S("professor.n.01")]
        )
        ...
        self.assertEqual(
            S("water.n.1").substance_meronyms(),
    ...
    def test_antonyms(self):
        ...
        self.assertEqual(
            L("increase.v.1.increase").antonyms(),
    ...
    def test_domains(self):
        # Test domains.
        self.assertEqual(
            S("code.n.03").topic_domains(), [S("computer_science.n.01")]
        )
    ...
    def test_in_topic_domains(self):
        ...
        self.assertEqual(
            S("germany.n.01").in_region_domains()[23], S("trillion.n.02")
        )
        self.assertEqual(
            S("slang.n.02").in_usage_domains()[1], S("airhead.n.01")
        )
    ...
    def test_wordnet_similarities(self):
        ...
        self.assertAlmostEqual(
            S("dog.n.01").jcn_similarity(S("cat.n.01"), brown_ic),
            0.4497,
            places=3,
        )
        semcor_ic = wnic.ic("ic-semcor.dat")
        self.assertAlmostEqual(
            S("dog.n.01").lin_similarity(S("cat.n.01"), semcor_ic),
            0.8863,
            places=3,
        )
    ...
    def test_omw_lemma_no_trailing_underscore(self):
        ...
        self.assertEqual(
            sorted(S("about-face.n.02").lemma_names(lang="slv")), expected
        )
    ...
    def test_iterable_type_for_all_lemma_names(self):
        ...
        self.assertTrue(
            hasattr(eng_lemmas, "__next__") or hasattr(eng_lemmas, "next")
        )
        self.assertTrue(eng_lemmas.__iter__() is eng_lemmas)

        self.assertTrue(hasattr(cat_lemmas, "__iter__"))
        self.assertTrue(
            hasattr(cat_lemmas, "__next__") or hasattr(eng_lemmas, "next")
        )
