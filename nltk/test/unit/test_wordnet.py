...
class WordnNetDemo(unittest.TestCase):
    ...
    def test_retrieve_synset(self):
        ...
        self.assertEqual(move_synset.definition(),
    ...
    def test_hyperhyponyms(self):
        ...
        first_five_meal_hypo = [S('banquet.n.02'), S('bite.n.04'), S(
            'breakfast.n.01'), S('brunch.n.01'), S('buffet.n.02')]
        self.assertEqual(sorted(S('meal.n.1').hyponyms()
                                [:5]), first_five_meal_hypo)
        self.assertEqual(S('Austen.n.1').instance_hypernyms(),
                         [S('writer.n.01')])
        first_five_composer_hypo = [S('ambrose.n.01'), S('bach.n.01'), S(
            'barber.n.01'), S('bartok.n.01'), S('beethoven.n.01')]
        self.assertEqual(S('composer.n.1').instance_hyponyms()
                         [:5], first_five_composer_hypo)
        ...
        self.assertEqual(S('fall.v.12').root_hypernyms(),
                         [S('act.v.01'), S('fall.v.17')])
    ...
    def test_meronyms_holonyms(self):
        ...
        self.assertEqual(S('dog.n.01').member_holonyms(), [
                         S('canis.n.01'), S('pack.n.06')])
    ...
    def test_antonyms(self):
        # Test antonyms.
        self.assertEqual(L('leader.n.1.leader').antonyms(),
                         [L('follower.n.01.follower')])
        self.assertEqual(L('increase.v.1.increase').antonyms(), [
                         L('decrease.v.01.decrease')])
    ...
    def test_domains(self):
        # Test domains.
        self.assertEqual(S('code.n.03').topic_domains(),
                         [S('computer_science.n.01')])
    ...
    def test_wordnet_similarities(self):
        ...
        self.assertAlmostEqual(
            S('cat.n.01').path_similarity(S('cat.n.01')), 1.0)
        self.assertAlmostEqual(
            S('dog.n.01').path_similarity(S('cat.n.01')), 0.2)
        self.assertAlmostEqual(S('dog.n.01').lch_similarity(
            S('cat.n.01')), 2.028, places=3)
        self.assertAlmostEqual(S('dog.n.01').wup_similarity(
            S('cat.n.01')), 0.8571, places=3)
        # Information Content similarities.
        brown_ic = wnic.ic('ic-brown.dat')
        self.assertAlmostEqual(S('dog.n.01').jcn_similarity(
            S('cat.n.01'), brown_ic), 0.4497, places=3)
        semcor_ic = wnic.ic('ic-semcor.dat')
        self.assertAlmostEqual(S('dog.n.01').lin_similarity(
            S('cat.n.01'), semcor_ic), 0.8863, places=3)
