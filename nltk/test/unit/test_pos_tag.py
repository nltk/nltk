...
class TestPosTag(unittest.TestCase):
    ...
    def test_pos_tag_eng_universal(self):
        ...
        assert (
            pos_tag(word_tokenize(text), tagset="universal") == expected_tagged
        )
    ...
    def test_pos_tag_unknown_lang(self):
        text = "모르겠 습니 다"
        self.assertRaises(
            NotImplementedError, pos_tag, word_tokenize(text), lang="kor"
        )
        # Test for default kwarg, `lang=None`
        self.assertRaises(
            NotImplementedError, pos_tag, word_tokenize(text), lang=None
        )
