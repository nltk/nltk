...
class TestCoreNLPServerError:
    ...
    def _dies_after(polls):
        class _Popen:
            ...
            def poll(self):
                self.calls += 1
                return 137 if self.calls > polls else None

            def communicate(self):
                return (None, "java.lang.OutOfMemoryError: Java heap space")

        return _Popen()

    @pytest.mark.parametrize("endpoint", ["live", "ready"])
    def test_a_jvm_that_dies_mid_wait_is_reported_at_once(self, monkeypatch, endpoint):
        # The process check runs on every poll, so a JVM that exits while the
        # liveness or readiness loop waits is reported with its exit code and
        # stderr immediately, not as "Could not connect" after the loop gives up.
        import requests
        ...
        popen = self._dies_after(polls=3)
        monkeypatch.setattr(m, "config_java", lambda *a, **k: None)
        monkeypatch.setattr(m, "java", lambda *a, **k: popen)
        monkeypatch.setattr(m.time, "sleep", lambda *_: None)
        monkeypatch.setattr(requests, "get", _get)
        srv = self._bare_server(["-quiet"])
        with pytest.raises(CoreNLPServerError, match="OutOfMemoryError") as info:
            srv.start()
        assert info.value.args[0] == 137
        assert len(gets) < 10  # stopped at the death, not after 30 or 60 polls

    def test_a_slow_but_alive_jvm_still_gets_the_full_wait(self, monkeypatch):
        # The new check must not shorten the wait for a server that is alive
        # but slow to bind: live answers only on the 25th poll and start succeeds.
        import requests
        ...
        monkeypatch.setattr(m, "config_java", lambda *a, **k: None)
        monkeypatch.setattr(m, "java", lambda *a, **k: _Alive())
        monkeypatch.setattr(m.time, "sleep", lambda *_: None)
        monkeypatch.setattr(requests, "get", _get)
        self._bare_server(["-quiet"]).start()
        assert count["live"] == 25
