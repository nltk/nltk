...
import nltk.parse.corenlp as corenlp_mod
...
def test_corenlp_server_start_uses_call_local_java_options(monkeypatch):
    captured = {}
    ...
    server = CoreNLPServer.__new__(CoreNLPServer)
    server.verbose = False
    server._classpath = ("stanford-corenlp.jar", "stanford-corenlp-models.jar")
    server.corenlp_options = []
    server.java_options = ["-XmxCORENLP"]
    server.url = "http://localhost:9000"

    monkeypatch.setattr(internals, "_java_options", ["-XmxGLOBAL"])

    def fake_java(
        cmd, classpath=None, blocking=True, stdout=None, stderr=None, options=None
    ):
        captured["options"] = options
    ...
    monkeypatch.setattr(corenlp_mod, "java", fake_java)
    monkeypatch.setattr("requests.get", fake_get)

    server.start()

    assert captured["options"] == ["-XmxCORENLP"]
    assert captured["blocking"] is False
    assert captured["health_urls"] == [
