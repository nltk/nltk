...
class MaceCommand(Prover9CommandParent, BaseModelBuilderCommand):
    ...
    def _make_relation_set(num_entities, values):
        ...
        for position in [pos for (pos, v) in enumerate(values) if v == 1]:
            r.add(tuple(MaceCommand._make_relation_tuple(
    ...
    def _make_relation_tuple(position, values, num_entities):
        if len(values) == 1:
            ...
            sublist = values[sublist_start *
                             sublist_size:(sublist_start + 1) * sublist_size]
...
def test_make_relation_set():
    print(MaceCommand._make_relation_set(num_entities=3,
                                         values=[1, 0, 1]) == set([('c',), ('a',)]))
    print(MaceCommand._make_relation_set(num_entities=3, values=[
          0, 0, 0, 0, 0, 0, 1, 0, 0]) == set([('c', 'a')]))
    print(MaceCommand._make_relation_set(num_entities=2, values=[
          0, 0, 1, 0, 0, 0, 1, 0]) == set([('a', 'b', 'a'), ('b', 'b', 'a')]))
