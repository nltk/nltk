...
class TableauProver(Prover):
    ...
    def _attempt_proof_some(self, current, context, agenda, accessible_vars, atoms, debug):
        ...
        agenda.put(current.term.replace(
            current.variable, new_unique_variable), context)
        agenda.mark_alls_fresh()
    ...
    def _attempt_proof_all(self, current, context, agenda, accessible_vars, atoms, debug):
        ...
        if accessible_vars:
            ...
            if bv_available:
                ...
                agenda.put(current.term.replace(
                    current.variable, variable_to_use), context)
...
class Agenda(object):
    ...
    def put(self, expression, context=None):
        ...
        self.sets[self._categorize_expression(
            ex_to_add)].add((ex_to_add, context))
