    ...
    def replace(self, variable, expression, replace_bound=False, alpha_convert=True):
        if self.variable == variable:
            assert isinstance(expression,
                              DrtAbstractVariableExpression), "Can only replace a proposition label with a variable"
            return DrtProposition(expression.variable,
...
class DrtBinaryExpression(DrtExpression, BinaryExpression):
    ...
    def _pretty(self):
        return DrtBinaryExpression._assemble_pretty(self._pretty_subex(self.first), self.getOp(),
    ...
    def _assemble_pretty(first_lines, op, second_lines):
        ...
        return ([' ' + first_line + ' ' + blank + ' ' + second_line + ' ' for first_line, second_line in
                 first_second_lines[:2]] +
                ['(' + first_line + ' ' + op + ' ' + second_line + ')' for first_line, second_line in
                 first_second_lines[2:3]] +
                [' ' + first_line + ' ' + blank + ' ' + second_line + ' ' for first_line, second_line in
                 first_second_lines[3:]])
...
class DrsDrawer(object):
    ...
    def _handle_BinaryExpression(self, expression, command, x, y):
        ...
        (right, first_bottom) = self._handle(expression.first, command, right,
        ...
        (right, second_bottom) = self._handle(expression.second, command, right,
