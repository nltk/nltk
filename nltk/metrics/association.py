...
class QuadgramAssocMeasures(NgramAssocMeasures):
    ...
    @staticmethod
    def _contingency(n_iiii, n_iiix_tuple, n_iixx_tuple, n_ixx_tuple, n_xxx):
        """Calculates values of a trigram contingency table (or cube) from
        marginal values.
        >>> TrigramAssocMeasures._contingency(1, (1, 1, 1), (1, 73, 1), 2000)
        (1, 0, 0, 0, 0, 72, 0, 1927)
        ...
        (n_ixxx, n_xixx, n_xxix, n_xxxi) = n_ixx_tuple
        n_oiii = n_xiii - n_iiii
        n_ioii = n_ixii - n_iiii
        n_ooii = n_xxii - n_iiii
        # n_oii = n_xii - n_iii
        # n_ioi = n_ixi - n_iii
        # n_iio = n_iix - n_iii
        # n_ooi = n_xxi - n_iii - n_oii - n_ioi
...
class QuadgramAssocMeasures(NgramAssocMeasures):
    ...
    #     return (n_iiii,
    #             (n_iiii + n_oiii, n_iiii + n_ioii, n_iiii + n_iioi),
    #             (n_iiii + n_ooii + n_iio + n_ioo,
    #              n_iiii + n_oii + n_iio + n_oio,
    #              n_iiii + n_oii + n_ioi + n_ooi),
    #             sum(contingency))
