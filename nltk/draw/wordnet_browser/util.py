...
#        typ = 'W'
    ...
    # XXX: What is prev_synset_key?
    descr = pos_tuple[2]
    ref = Reference(word, synset_key)
...
def _collect_all_synsets(word, pos):
    ...
    """
    return '<ul>%s\n</ul>\n' % \
        ''.join((_collect_one_synset(word, synset, '') for synset in wordnet.synsets(word, pos)))
...
class Reference(object):
    ...
    def __init__(self, word, synset_key=None):
        self.word = word
        self.synset_key = synset_key
    ...
    def encode(self):
        ...
        string = cPickle.dumps((self.word, self.synset_key), -1)
...
def decode_reference(string):
    ...
    word, synset_key = cPickle.loads(string)
    return Reference(word, synset_key)
