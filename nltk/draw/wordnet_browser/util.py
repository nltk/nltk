...
def _collect_one_synset(word, synset, synset_keys):
    '''
...
#    else: # It's a synset
    ...
    return s + gl + _synset_relations(word, synset, synset_keys) + '</li>\n'

def _collect_all_synsets(word, pos, synset_keys=set()):
    """
    Return a HTML unordered list of synsets for the given word and
    part of speach.
    """
    return '<ul>%s\n</ul>\n' % \
        ''.join((_collect_one_synset(word, synset, synset_keys) for synset in wordnet.synsets(word, pos)))
...
def _synset_relations(word, synset, synset_keys):
    '''
    ...
    @type synset: Synset
    @param synset_keys: synset keys for synsets that should have relations displayed.
    @type synset_keys: set(synset_key)
    @return: The HTML for a synset's relations
    @rtype: str
    '''

    if not synset.name in synset_keys:
        return ""
...
class Reference(object):
    ...
    def __init__(self, word, synset_keys=set()):
        """
        ...
        synset_keys is a set of synset keys to unfold a list of
        synset relations for.
        """
        self.word = word
        self.synset_keys = synset_keys
    ...
    def encode(self):
        ...
        string = cPickle.dumps((self.word, self.synset_keys), -1)
...
def decode_reference(string):
    ...
    word, synset_keys = cPickle.loads(string)
    return Reference(word, synset_keys)
    ...
    for pos,pos_str,name in _pos_tuples:
        if pos in pos_forms:
            ...
            for w in pos_forms[pos]:
                ...
                # to the next word if a KeyError is raised.
                try:
                    body += _collect_all_synsets(w, pos, href.synset_keys)
                except KeyError:
                    pass
    if not body:
        body = "The word or words '%s' where not found in the dictonary." % word
