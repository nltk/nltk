...
class ConcordanceSearchView(object):
    ...
    def _init_widgets(self, parent):
        self.main_frame = Frame(parent, dict(
            background=self._BACKGROUND_COLOUR, padx=1, pady=1, border=1))
    ...
    def _init_corpus_select(self, parent):
        ...
        om = OptionMenu(innerframe, self.var, self.model.DEFAULT_CORPUS,
                        command=self.corpus_selected, *self.model.non_default_corpora())
        om['borderwidth'] = 0
    ...
    def _init_status(self, parent):
        self.status = Label(parent, justify=LEFT, relief=SUNKEN,
                            background=self._BACKGROUND_COLOUR, border=0, padx=1, pady=0)
    ...
    def _init_query_box(self, parent):
        ...
        self.search_button = Button(
            another, text='Search', command=self.search, borderwidth=1, highlightthickness=1)
        self.search_button.pack(side='left', fill='x',
                                pady=25, anchor='center')
    ...
    def _init_paging(self, parent):
        innerframe = Frame(parent, background=self._BACKGROUND_COLOUR)
        self.prev = prev = Button(innerframe, text='Previous', command=self.previous,
                                  width='10', borderwidth=1, highlightthickness=1, state='disabled')
        prev.pack(side='left', anchor='center')
        self.next = next = Button(innerframe, text='Next', command=self.__next__,
                                  width='10', borderwidth=1, highlightthickness=1, state='disabled')
    ...
    def write_results(self, results):
        ...
        for each in results:
            ...
            if len(sent) != 0:
                ...
                sentence = sent[pos1 -
                                self._char_before:pos1 + self._char_after]
                ...
                for marker in word_markers:
                    self.results_box.tag_add(self._HIGHLIGHT_WORD_TAG, str(
                        row) + '.' + str(marker[0]), str(row) + '.' + str(marker[1]))
                for marker in label_markers:
                    self.results_box.tag_add(self._HIGHLIGHT_LABEL_TAG, str(
                        row) + '.' + str(marker[0]), str(row) + '.' + str(marker[1]))
                row += 1
