...
_CHART_GRAMMAR_ALLOWED_GLOBALS = (
    ...
    # Grammar, productions and (non)terminals.
    ("nltk.grammar", "CFG"),
    ("nltk.grammar", "PCFG"),
    ...
    # Parse trees (a chart / grammar may carry any of the Tree variants).
    ("nltk.tree.tree", "Tree"),
    ("nltk.tree.immutable", "ImmutableTree"),
    ("nltk.tree.probabilistic", "ProbabilisticTree"),
    ("nltk.tree.immutable", "ImmutableProbabilisticTree"),
    ("nltk.tree.parented", "ParentedTree"),
    ("nltk.tree.parented", "MultiParentedTree"),
    # Feature structures and their value types (feature grammars / charts).
    ...
    # Feature values that are logic variables (e.g. ``?n`` reentrancies).
    ("nltk.sem.logic", "Variable"),
)


def _load_chart_pickle(file):
    """Unpickle a chart / grammar file through an allowlisting unpickler.

    ``file`` is an open binary handle to an operator-chosen ``.pickle`` file.
    Only the exact grammar / chart / tree / feature classes a legitimate file
    reconstructs (:data:`_CHART_GRAMMAR_ALLOWED_GLOBALS`) may be built; every
    other global raises :class:`pickle.UnpicklingError` before it can execute,
    so a reduce gadget such as ``os.system`` in a malicious file is refused
    instead of run (the warn-only ``pickle_load`` this replaces executed it).
    """
    return AllowlistUnpickler(
        file, allowed_globals=_CHART_GRAMMAR_ALLOWED_GLOBALS
    ).load()
