...
######################################################################
# Lazy Mapped List (e.g., for feature detection)
######################################################################
class LazyMappedList(object):
    """
    A read-only list of the values generated by applying a function to
    an underlying list.  The function is applied lazily -- i.e., when
    you read a value from the list, C{LazyMappedList} will calculate
    that value by applying its function to the underlying list's value.
    ...
    using a C{LazyMappedList} can significantly reduce memory usage
    when training and running classifiers.
    """
    ...
    def __getitem__(self, index):
        if isinstance(index, slice):
            return [self._func(v) for v in self._lst[index]]
        else:
            return self._func(self._lst[index])

    def __iter__(self):
        return (self._func(v) for v in self._lst)
...
class LazyMappedChain(object):
    ...
    def __getitem__(self, index):
        if isinstance(index, slice):
            R = range(*index.indices(len(self)))
        else:
            R = [index]
        i = -1
