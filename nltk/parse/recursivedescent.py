...
class RecursiveDescentParser(ParserI):
    ...
    def __init__(self, grammar, trace=0, max_steps=None):
        """
        ...
        :param max_steps: Maximum number of parse steps (recursive calls)
            before raising ``RuntimeError``.  Limits exponential blowup
            from highly ambiguous or adversarial grammars.  ``None``
    ...
    def parse(self, tokens):
        ...
        step_counter = [0]
        return self._parse(tokens, initial_tree, frontier, step_counter)

    def _parse(self, remaining_text, tree, frontier, step_counter=None):
        """
        ...
        :type step_counter: list(int) or None
        :param step_counter: Mutable single-element list used to track
            total recursive calls.  Incremented on each entry; raises
            ``RuntimeError`` when ``self._max_steps`` is exceeded.
        """
        if step_counter is None:
            step_counter = [0]
        step_counter[0] += 1
        if self._max_steps is not None and step_counter[0] > self._max_steps:
            raise RuntimeError(
                "RecursiveDescentParser exceeded max_steps={} — "
                "the grammar may be highly ambiguous or adversarial.".format(
                    self._max_steps
                )
            )
        ...
        # If the next element on the frontier is a tree, expand it.
            ...
            yield from self._expand(remaining_text, tree, frontier, step_counter)

        # If the next element on the frontier is a token, match it.
        else:
            yield from self._match(remaining_text, tree, frontier, step_counter)

    def _match(self, rtext, tree, frontier, step_counter=None):
        """
        :rtype: iter(Tree)
        :return: an iterator of all parses that can be generated by
        ...
        if len(rtext) > 0 and tree_leaf == rtext[0]:
            ...
            yield from self._parse(rtext[1:], newtree, frontier[1:], step_counter)
        else:
            # If it's a non-matching terminal, fail.
            if self._trace:
                self._trace_backtrack(tree, frontier, rtext[:1])

    def _expand(self, remaining_text, tree, frontier, step_counter=None, production=None):
        """
        :rtype: iter(Tree)
        :return: An iterator of all parses that can be generated by
        ...
        for production in productions:
            ...
            if lhs == tree[frontier[0]].label():
                ...
                yield from self._parse(
                    remaining_text, newtree, new_frontier + frontier[1:], step_counter
                )
...
class SteppingRecursiveDescentParser(RecursiveDescentParser):
    ...
    def expand(self, production=None):
        ...
        for prod in productions:
            ...
            # Try expanding.
            for _result in self._expand(self._rtext, self._tree, self._frontier, production=prod):
                return prod
