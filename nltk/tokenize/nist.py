...
class NISTTokenizer(TokenizerI):
    ...
    pup_number = text_type(
        ''.join(set(perluniprops.chars('Number'))))  # i.e. \p{N}
    pup_punct = text_type(
        ''.join(set(perluniprops.chars('Punctuation'))))  # i.e. \p{P}
    pup_symbol = text_type(
        ''.join(set(perluniprops.chars('Symbol'))))  # i.e. \p{S}
    ...
    PUNCT_1 = re.compile(u"([{n}])([{p}])".format(
        n=number_regex, p=punct_regex)), '\\1 \\2 '
    PUNCT_2 = re.compile(u"([{p}])([{n}])".format(
        n=number_regex, p=punct_regex)), ' \\1 \\2'
    # Tokenize symbols
