...
class ToktokTokenizer(TokenizerI):
    ...
    FUNKY_PUNCT_1 = re.compile('([،;؛¿!"\])}»›”؟¡%٪°±©®।॥…])'), r" \1 "
    # Pad more funky punctuation.
    FUNKY_PUNCT_2 = re.compile("([({\[“‘„‚«‹「『])"), r" \1 "
    ...
    OPEN_PUNCT_RE = re.compile("([{}])".format(OPEN_PUNCT)), r"\1 "
    # Pad spaces before closing punctuations.
    CLOSE_PUNCT_RE = re.compile("([{}])".format(CLOSE_PUNCT)), r"\1 "
    # Pad spaces after currency symbols.
    CURRENCY_SYM_RE = re.compile("([{}])".format(CURRENCY_SYM)), r"\1 "
