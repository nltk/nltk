...
class RegexpTokenizer(TokenizerI):
    ...
    Unlike ``re.findall()`` and ``re.split()``, ``RegexpTokenizer`` does
    not treat regular expressions that contain grouping parentheses specially.
    """
    def __init__(self, pattern, gaps=False, discard_empty=True,
        ...
        regular expression *pattern*.  By default, *pattern* will be
        used to find tokens; but if ``gaps == False``, then
        *patterns* will be used to find separators between tokens
        instead.  E.g.
