...
from nltk.internals import compile_regexp_to_noncapturing, has_capturing_groups
...
class RegexpTokenizer(TokenizerI):
    ...
    :type compat: bool
    :param compat: True if the old behavior of removing grouping
        parentheses is required (default=False)
    :type flags: int
    ...
    """
    def __init__(self, pattern, gaps=False, discard_empty=True, compat=False,
    ...
    def _check_regexp(self):
        if self._regexp is None:
            ...
            elif has_capturing_groups(self._pattern):
                raise ValueError('Error in regular expression %r: to support capturing groups, initialize tokenizer with compat=True' % self._pattern)
            else:
