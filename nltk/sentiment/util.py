...
def parse_tweets_set(filename, label, word_tokenizer, sent_tokenizer=None,
    ...
    if sys.version_info[0] == 3:
        with codecs.open(filename, 'rt') as csvfile:
            ...
            if skip_header == True:
                next(reader, None) # skip the header generated by json2csv
            i = 0
            for tweet_id, text, lang in reader:
                # text = text[1]
                i += 1
...
def demo_tweets(trainer):
    '''
    Train Naive Bayes classifier on 8000 instances of tweets dataset,
    ...
    pos_docs = parse_tweets_set('positive_tweets.csv', label='pos', word_tokenizer=tokenizer)
    neg_docs = parse_tweets_set('negative_tweets.csv', label='neg', word_tokenizer=tokenizer)
