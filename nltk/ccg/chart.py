    ...
    def apply(self, chart, grammar, left_edge, right_edge):
        ...
        if self._combinator.can_combine(left_edge.categ(), right_edge.categ()):
            for res in self._combinator.combine(left_edge.categ(), right_edge.categ()):
                new_edge = CCGEdge(
                    span=(left_edge.start(), right_edge.end()), categ=res, rule=self._combinator)
...
class ForwardTypeRaiseRule(AbstractChartRule):
    ...
    def apply(self, chart, grammar, left_edge, right_edge):
        ...
        for res in self._combinator.combine(left_edge.categ(), right_edge.categ()):
            new_edge = CCGEdge(span=left_edge.span(),
                               categ=res, rule=self._combinator)
...
class BackwardTypeRaiseRule(AbstractChartRule):
    ...
    def apply(self, chart, grammar, left_edge, right_edge):
        ...
        for res in self._combinator.combine(left_edge.categ(), right_edge.categ()):
            new_edge = CCGEdge(span=right_edge.span(),
                               categ=res, rule=self._combinator)
    ...
    def _trees(self, edge, complete, memo, tree_class):
        ...
        for cpl in self.child_pointer_lists(edge):
            ...
            for children in itertools.product(*child_choices):
                lhs = (Token(self._tokens[edge.start():edge.end()], edge.lhs(
                ), compute_semantics(children, edge)), str(edge.rule()))
