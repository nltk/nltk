...
def _pretty_lu(lu):
    ...
    if 'semTypes' in lukeys:
        ...
        outstr += "  " * (len(lu.semTypes) > 0) + ", ".join(
            '{0}({1})'.format(x.name, x.ID) for x in lu.semTypes) + '\n' * (len(lu.semTypes) > 0)
    if 'URL' in lukeys:
        outstr += "\n[URL] {0}\n".format(lu.URL)
...
def _annotation_ascii_frames(sent):
    ...
    for j, k, fname, asetIndex in overt:
        if not j >= i:
            assert j >= i, ('Overlapping targets?' + (
            ' UNANN' if any(aset.status == 'UNANN' for aset in sent.annotationSet[1:]) else ''), (j, k, asetIndex))
        s1 += ' ' * (j - i) + '*' * (k - j)
        short = fname[:k - j]
        if (k - j) < len(fname):
            r = 0
...
def _annotation_ascii_FEs(sent):
    ...
    FE1 = _annotation_ascii_FE_layer(sorted(sent.FE[0] + (posspec if not posspec_separate else [])), sent.FE[1],
                                     feAbbrevs)
    FE2 = FE3 = None
    if 'FE2' in sent:
...
def _pretty_frame(frame):
    ...
    outstr += "  " * (len(frame.semTypes) > 0) + ", ".join(
        "{0}({1})".format(x.name, x.ID) for x in frame.semTypes) + '\n' * (len(frame.semTypes) > 0)
    ...
    for ct in sorted(fes.keys(),
                     key=lambda ct2: ['Core', 'Core-Unexpressed', 'Peripheral', 'Extra-Thematic'].index(ct2)):
...
warnings(True) to display corpus consistency warnings when loading data
    ...
    def frame_ids_and_names(self, name=None):
        ...
        if not self._frame_idx:
            self._buildframeindex()
        return dict((fID, finfo.name) for fID, finfo in self._frame_idx.items() if
                    name is None or re.search(name, finfo.name) is not None)
    ...
    def annotations(self, luNamePattern=None, exemplars=True, full_text=True):
        ...
        if full_text:
            ...
            ftpart = PrettyLazyIteratorList(aset for sent in self.ft_sents() for aset in sent.annotationSet[1:] if
                                            luNamePattern is None or aset.get('luID', 'CXN_ASET') in matchedLUIDs)
        else:
            ftpart = []
    ...
    def exemplars(self, luNamePattern=None, frame=None, fe=None, fe2=None):
        ...
        def _matching_exs():
            for f in frames:
                ...
                if fe is not None:
                    fes = {ffe for ffe in f.FE.keys() if re.search(fe, ffe, re.I)} if isinstance(fe,
                                                                                                 six.string_types) else {
                    fe.name}
                    if fe2 is not None:
                        fes2 = {ffe for ffe in f.FE.keys() if re.search(fe2, ffe, re.I)} if isinstance(fe2,
                                                                                                       six.string_types) else {
                        fe2.name}
    ...
    def fe_relations(self):
        ...
        return PrettyList(sorted(self._ferel_idx.values(),
                                 key=lambda ferel: (ferel.type.ID, ferel.frameRelation.superFrameName,
                                                    ferel.superFEName, ferel.frameRelation.subFrameName,
                                                    ferel.subFEName)))
                ...
                if l is not None:
                    ...
                    for lbl in l.label:
                        if 'start' in lbl:
                            ...
                            if l.name not in (
                            'Sent', 'Other'):  # 'Sent' and 'Other' layers sometimes contain accidental duplicate spans
