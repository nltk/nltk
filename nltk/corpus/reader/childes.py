...
class CHILDESCorpusReader(XMLCorpusReader):
    ...
    def words(self, fileids=None, speaker='ALL', stem=False,
        ...
        def get_words(fileid): return self._get_words(fileid, speaker, sent, stem, relation,
    ...
    def age(self, fileids=None, speaker='CHI', month=False):
        ...
        def get_age(fileid): return self._get_age(fileid, speaker, month)
    ...
    def MLU(self, fileids=None, speaker='CHI'):
        ...
        def get_MLU(fileid): return self._getMLU(fileid, speaker=speaker)
    ...
    def webview_file(self, fileid, urlbase=None):
        ...
        if urlbase:
            ...
            if '/childes/' in full.lower():
                ...
                path = re.findall(
                    r'(?i)/childes(?:/data-xml)?/(.*)\.xml', full)[0]
            elif 'eng-usa' in full.lower():
                path = 'Eng-USA/' + \
                    re.findall(r'/(?i)Eng-USA/(.*)\.xml', full)[0]
            else:
                path = fileid
...
def demo(corpus_root=None):
    ...
    try:
        ...
        for file in childes.fileids()[:5]:
            ...
            print("words (only MOT):", childes.words(
                file, speaker='MOT')[:7], "...")
            print("words (only CHI):", childes.words(
                file, speaker='CHI')[:7], "...")
            print("stemmed words:", childes.words(file, stem=True)[:7], " ...")
            print("words with relations and pos-tag:",
