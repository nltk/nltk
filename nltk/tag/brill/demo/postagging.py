...
def postag(
    ...
    :param cache_baseline_tagger: the file where rules will be saved
    ...
    if templates is None:
        ...
        ## you could also try one of the pre-built sets, available through
    ...
    # The data used to train the baseline tagger should be disjoint from that for the rule learner
    # (and both disjoint from the test, of course). For pedagogical reasons, one can see what happens
    # otherwise with separate_baseline_data=False: only a few rules learned, and better baseline
    # but worse end result on unseen data.
    ...
    if  incremental_stats:
        print("Incrementally tagging the test data and collecting individual rule statistics")
