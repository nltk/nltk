...
class FreqDist(Counter):
    ...
    def plot(self, *args, **kwargs):
        ...
        pylab.xticks(range(len(samples)), [text_type(s)
                                           for s in samples], rotation=90)
    ...
    def __ge__(self, other): return not self <= other or self == other
    ...
    def pformat(self, maxlen=10):
        ...
        :rtype: string
        """
        items = ['{0!r}: {1!r}'.format(*item)
                 for item in self.most_common(maxlen)]
        if len(self) > maxlen:
            items.append('...')
...
class ConditionalFreqDist(defaultdict):
    ...
    def plot(self, *args, **kwargs):
        ...
        cumulative = _get_kwarg(kwargs, 'cumulative', False)
        conditions = _get_kwarg(kwargs, 'conditions',
...
def gt_demo():
    ...
    fd_keys_sorted = (key for key, value in sorted(
