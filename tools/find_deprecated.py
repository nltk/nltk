    ...
    for path in sorted(paths):
        if os.path.isdir(path):
            ...
            print_deprecated_uses_in(
                open(path).readline, path, dep_files, dep_names, 0
            )
...
def print_deprecated_uses_in(
    readline, path, dep_files, dep_names, lineno_offset
):
    ...
    for (typ, tok, start, end, line) in tokiter:
        ...
        if not (
            ...
            or (
                tok in deprecated_funcs
                and re.search(r"\b{}\s*\(".format(esctok), line)
            )
            or (
