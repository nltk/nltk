...
def morphy(form, pos=NOUN):
    '''A generator that produces the base forms for a form of a given POS.
    First it checks if the word is found in the exception list for this POS.
    If it is, it yields all the exception's base forms, one by one.

    After that it starts a recursive generator with the form and a list of
    suffix substitutions for that POS. In every call it first checks if the
    form is found in the dictionary and if it is, it yields the form.
    ...
    new and doing a recursive call of the generator.
...
def p(word):
    ...
    for pos in [NOUN, VERB, ADJECTIVE, ADVERB]:
        for form in morphy(word, pos=pos):
