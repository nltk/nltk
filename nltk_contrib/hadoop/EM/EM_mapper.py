...
class EM_Mapper(MapperBase):
	...
	def map(self, key, value):
		...
		for i in range(N):
			self.outputcollector.collect("parameters", tuple2str(("Pi", states[i], pi.prob(states[i]))))
		...
		self.outputcollector.collect("parameters", "states " + tuple2str(tuple(states)))
		self.outputcollector.collect("parameters", "symbols " + tuple2str(tuple(symbols)))
	...
	def collect_matrix(self, name, matrix, lpk, row, col):
		...
		for i in range(row):
			for j in range(col):
				self.outputcollector.collect("parameters", tuple2str((name, i, j, \
						matrix[i][j], lpk, row, col)))
