...
class Category(Nonterminal):
    ...
    A C{Category} acts like a dictionary, except in the following ways:
        ...
        - Subclasses of C{Category} may contain a list of I{required features},
          which are names of features whose value is C{False} if unspecified. A
          ...
          C{False}, it is considered to be not present. (Mixing different
    ...
    def feature_names(self):
        ...
        """
        return filter(lambda x: not (x in self._required and self[x] is False),
        self._features.keys())
    ...
    def parse(cls, s):
        ...
        if position != len(s):
            raise ValueError, ("Couldn't parse %s: expected end of string\n%s\n%s" % (cls.__name__, s, ' '*position + '^'))
        return cls(parsed)
    ...
    def _parse(cls, s, position=0, reentrances=None):
        ...
        if position >= len(s) or s[position] != '[':
            return {cls.headname: head}, position
        position += 1
        ...
        if match is not None:
            if head is None: return {}, match.end()
            else: return {cls.headname: head}, match.end()
...
def demo():
    from nltk_lite.parse import category, GrammarFile
