...
class FeatureStruct:
    ...
    A feature structure's feature values can be accessed via indexing:
      ...
      >>> fstruct1 = FeatureStruct(number='singular', person='3rd')
      >>> print fstruct1['number']
      'singular'

      >>> fstruct2 = FeatureStruct(subject=fstruct1)
    ...
    def __getitem__(self, index):
        ...
        elif isinstance(self._features[index[0]], FeatureStruct):
    ...
    def equal_values(self, other, check_reentrance=False):
        ...
        """
        if not isinstance(other, FeatureStruct): return 0
        ...
        for (fname, selfval) in self._features.items():
            ...
            if isinstance(selfval, FeatureStruct):
    ...
    def unify(self, other, bindings=None, trace=False):
        ...
        except FeatureStruct._UnificationFailureError: return None
    ...
    def _apply_forwards_to_bindings(self, bindings):
        ...
        for var in bindings.bound_variables():
            ...
            if (isinstance(value, FeatureStruct) and
    ...
    def parse(s):
        ...
        displayed by repr) into a C{FeatureStruct}.  This parse
        ...
        """
        try:
            value, position = FeatureStruct._parse(s, 0, {})
        except ValueError, e:
    ...
    def _parse(s, position=0, reentrances=None):
        ...
        _PARSE_RE = FeatureStruct._PARSE_RE
        ...
        if match is not None: return FeatureStruct(), match.end()
        ...
        while position < len(s):
            ...
            # If it's not a reentrance link, it must be an assignment.
                ...
                val, position = FeatureStruct._parseval(s, position,
                                                           reentrances)
                features[name] = val
                if id is not None:
                    reentrances[id] = val
            ...
            match = _PARSE_RE['bracket'].match(s, position)
            if match is not None:
                return FeatureStruct(**features), match.end()
    ...
    def _parseval(s, position, reentrances):
        ...
        if s[position] == '[':
            return FeatureStruct._parse(s, position, reentrances)
...
# Note: since FeatureStruct.__repr__() sorts by keys before
# displaying, there is a single unique string repr for each
# FeatureStruct.
class FeatureStructTestCase(unittest.TestCase):
    'Unit testing for FeatureStruct'

    def testUnification(self):
        'Basic unification tests'

        # Copying from self to other.
        fs1 = FeatureStruct(number='singular')
        fs2 = fs1.unify(FeatureStruct())
        self.failUnlessEqual(repr(fs2), "[number='singular']")

        # Copying from other to self
        fs1 = FeatureStruct()
        fs2 = fs1.unify(FeatureStruct(number='singular'))
        self.failUnlessEqual(repr(fs2), "[number='singular']")

        # Cross copying
        fs1 = FeatureStruct(number='singular')
        fs2 = fs1.unify(FeatureStruct(person=3))
        self.failUnlessEqual(repr(fs2), "[number='singular', person=3]")

        # Merging a nested structure
        fs1 = FeatureStruct.parse('[A=[B=b]]')
        fs2 = FeatureStruct.parse('[A=[C=c]]')
        fs3 = fs1.unify(fs2)
    ...
    def testReentrantUnification(self):
        ...
        fs1 = FeatureStruct.parse('[A=(1)[B=b], E=[F->(1)]]')
        fs2 = FeatureStruct.parse("[A=[C='c'], E=[F=[D='d']]]")
        fs3 = fs1.unify(fs2)
        ...
        fs1 = FeatureStruct.parse("[a=[],b=[],c=[],d=[]]")
        fs2 = FeatureStruct.parse('[a=(1)[], b->(1), c->(1), d->(1)]')
        fs3 = fs1.unify(fs2)
        self.failUnlessEqual(repr(fs3), '[a=(1)[], b->(1), c->(1), d->(1)]')

        # fs1[a] gets unified with itself:
        fs1 = FeatureStruct.parse('[x=(1)[], y->(1)]')
        fs2 = FeatureStruct.parse('[x=(1)[], y->(1)]')
        fs3 = fs1.unify(fs2)
        
    def testVariableForwarding(self):
        'Bound variables should get forwarded appropriately'
        fs1 = FeatureStruct.parse('[A=(1)[X=x], B->(1), C=?cvar, D=?dvar]')

        fs2y = FeatureStruct(Y='y')
        fs2z = FeatureStruct(Z='z')
        fs2 = FeatureStruct.parse('[A=(1)[Y=y], B=(2)[Z=z], C->(1), D->(2)]')
    ...
    def testCyclicStructures(self):
        ...
        fs1 = FeatureStruct.parse('[F=(1)[], G->(1)]')
        fs2 = FeatureStruct.parse('[F=[H=(2)[]], G->(2)]')
        fs3 = fs1.unify(fs2)
        ...
        x = FeatureVariable('x')
        fs1 = FeatureStruct(F=FeatureStruct(H=x))
        fs2 = FeatureStruct(F=x)
        fs3 = fs1.unify(fs2)
    ...
    def testVariablesPreserveReentrance(self):
        ...
        fs1 = FeatureStruct.parse("[a=?x]")
        fs2 = fs1.unify(FeatureStruct.parse("[a=[]]"), bindings)
        fs3 = fs2.unify(FeatureStruct.parse("[b=?x]"), bindings)
...
def demo(trace=False):
    ...
    all_fstructs = [(i, FeatureStruct.parse(fstruct_strings[i]))
