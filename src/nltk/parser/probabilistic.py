...
class ProbabilisticParserI(ParserI):
    ...
    parse, the trees generated by C{ProbabilisticParser} all define
    the C{PROB} property.
    ...
    @outprop: C{TREE}: The parse tree, encoded as a
              L{ProbabilisticTreeToken}.  I{(generated by L{parse})}
    @outprop: C{TREES}: A list of possible parse trees, encoded as
              L{ProbabilisticTreeTokens}.  I{(generated by L{parse_n})}
    @outprop: C{TREEDIST}: A probability distribution over parse
              trees, with samples encoded as
              L{ProbabilisticTreeTokens}.  I{(generated by
              L{parse_dist})}
    @outprop: C{PROB}: The probability of the generated trees.
    """
    def parse_dist(self, token):
        """
        Find all parses for the given token, and output their
        probability distribution to the token's C{TREEDIST} property.

        @param token: The token whose subtokens should be parsed.
        @type token: L{Token}
        """
        
class AbstractProbabilisticParser(ProbabilisticParserI, AbstractParser):
    """
    An abstract base class for parsers.  C{AbstractProbabilisticParser}
    provides default implementations for:
      - L{parse_n} (based on C{parse})
      - L{parse_dist} (based on C{parse_n})

    It also provides L{_parse_from_parse_n}, which can be used to
    implement C{parse} based on C{parse_n}; and
    L{_parse_from_parse_dist}, which can be used to implement C{parse}
...
class ViterbiPCFGParser(AbstractProbabilisticParser):
    """
...
class BottomUpPCFGChartParser(AbstractProbabilisticParser):
    """
