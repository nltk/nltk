...
class ShiftReduceParser(AbstractParser):
    ...
    def parse(self, token):
        ...
        stack = []
        remaining_text = token[subtokens_prop][:]
        
        # Trace output.
        if self._trace:
            print 'Parsing %r' % ' '.join([tok[leaf_prop]
                                           for tok in token[subtokens_prop]])
        ...
        if stack[0][node_prop] != self._grammar.start().symbol():
            return None
        
        # We parsed successfully!
        token[tree_prop] = stack[0]
    ...
    def _match_rhs(self, rhs, rightmost_stack):
        ...
        for i in range(len(rightmost_stack)):
            if isinstance(rightmost_stack[i], TreeToken):
                ...
                if rightmost_stack[i][node_prop] != rhs[i].symbol(): return 0
            else:
                if isinstance(rhs[i], Nonterminal): return 0
                if rightmost_stack[i][leaf_prop] != rhs[i]: return 0
        return 1
    ...
    def _reduce(self, stack, remaining_text, production=None):
        ...
        for production in productions:
            ...
            if self._match_rhs(production.rhs(), stack[-rhslen:]):
                ...
                treetok = TreeToken(**{node_prop: production.lhs().symbol(),
    ...
    def _trace_stack(self, stack, remaining_text, marker=' '):
        ...
        for tok in stack:
            if isinstance(tok, TreeToken):
                str += `Nonterminal(tok[node_prop])` + ' '
            else:
                str += `tok[leaf_prop]` + ' '
        str += '* ' + ' '.join([`s[leaf_prop]` for s in remaining_text]) + ']'
        print str
...
class SteppingShiftReduceParser(ShiftReduceParser):
    ...
    def parses(self):
        ...
        if self._stack[0][node_prop] != self._grammar.start().symbol():
            return []
        return self._stack
