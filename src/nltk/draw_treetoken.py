...
class Draw_Node:
    ...
    def __init__(self, node):
        ...
        self.yloc = 0
        self.arrow_list = []
        self.loc = node.location() # returns a Location
    ...
    def display_collapsed(self, canvas, x, depth, left):
        ...
        self.collapsed_string = canvas.create_text(x, (depth+1)*50,
                                                   text = self.string())
        self.span = canvas.create_text(x, (depth+1)*50+15, 
                                       text='['+str(self.loc.start())+','+str(self.loc.end())+']')                                                
        bbox = canvas.bbox(self.collapsed_string, self.span)
        i = bbox[0]
        j = bbox[2]
        if left > i :
            diff = left-i
            canvas.move(self.collapsed_string, diff, 0)
            canvas.move(self.span, diff, 0)
            j += diff
            i += diff
        j += 4
        parentx = (j+left)/2
...
class Draw_Token:
    """
    a purely structural class, Draw_Token stores all important information
    ...
    def __init__(self, token):
        ...
        self.span = None # will store ther tag for the other span text
        self.xloc = 0
        self.yloc = 0
