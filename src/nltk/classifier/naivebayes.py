...
#    C{P(f1=0, f2=0, ..., fn=0)}.  
#    """
    def __init__(self, fdlist, labels, label_probdist, fval_probdist):
        """
        ...
        """
        assert _chktype(1, fdlist, FeatureDetectorListI)
    ...
    def fvlist_likelihood(self, fvlist, label):
        ...
        #DEBUG = '%20s' % label
        for fid in range(len(fvlist)):
            #z = self._fval_probdist[label,fid].prob(fvlist[fid])
            #DEBUG += '%10.5f' % z
            p *= self._fval_probdist[label,fid].prob(fvlist[fid])
...
class NBClassifierTrainer(ClassifierTrainerI):
    ...
    def train(self, labeled_tokens, **kwargs):
        ...
        for labeled_token in labeled_tokens:
            ...
            for fid in range(len(fvlist)):
                fval_freqdist[label, fid].inc(fvlist[fid])
        ...
        return NBClassifier(self._fdlist, labels,
...
def demo(labeled_tokens, n_words=5, n_lens=20, debug=1):
    ...
    fdlist = SimpleFDList(feature_detectors)
