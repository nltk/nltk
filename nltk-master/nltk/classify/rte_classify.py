...
class RTEFeatureExtractor(object):
    ...
    def __init__(self, rtepair, stop=True, lemmatize=False):
        ...
        """
        self.stop = stop
        self.stopwords = {'a', 'the', 'it', 'they', 'of', 'in', 'to', 'have', 'is', 'are', 'were', 'and', 'very', '.',
                          ','}
...
def rte_features(rtepair):
    ...
    features = {'alwayson': True, 'word_overlap': len(extractor.overlap('word')),
                'word_hyp_extra': len(extractor.hyp_extra('word')), 'ne_overlap': len(extractor.overlap('ne')),
                'ne_hyp_extra': len(extractor.hyp_extra('ne')),
                'neg_txt': len(extractor.negwords & extractor.text_words),
                'neg_hyp': len(extractor.negwords & extractor.hyp_words)}
    return features
