    ...
    def prob(self, sample):
        ...
        if count == 0:
            if self._bins == self._freqdist.B():
                ...
                p /= 1.0 * self._bins - self._freqdist.B()
        else:
            p *= self._renormal
        return p
...
class ConditionalFreqDist(defaultdict):
    ...
    def plot(self, linewidth, label, *args, **kwargs):
        """
...
def gt_demo():
    ...
    for key in fd:
        print('{0:>18s} {1:8d}  {2:12e}   {3:14e}   {4:12e}' \
            .format(key, fd[key], sgt.prob(key), katz.prob(key)))
