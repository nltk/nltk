...
def default_reasoning_demo():
    ...
    premises = [lp.parse(r'all x.(elephant(x)        -> animal(x))'),
                lp.parse(r'all x.(bird(x)            -> animal(x))'),
                lp.parse(r'all x.(dove(x)            -> bird(x))'), lp.parse(r'all x.(ostrich(x)         -> bird(x))'),
                lp.parse(r'all x.(flying_ostrich(x)  -> ostrich(x))'),
                lp.parse(r'all x.((animal(x)  & -Ab1(x)) -> -fly(x))'),
                lp.parse(r'all x.((bird(x)    & -Ab2(x)) -> fly(x))'),
                lp.parse(r'all x.((ostrich(x) & -Ab3(x)) -> -fly(x))'),
                lp.parse(r'all x.(bird(x)           -> Ab1(x))'), lp.parse(r'all x.(ostrich(x)        -> Ab2(x))'),
                lp.parse(r'all x.(flying_ostrich(x) -> Ab3(x))'), lp.parse(r'elephant(E)'), lp.parse(r'dove(D)'),
                lp.parse(r'ostrich(O)')]
