...
class FrenchStemmer(_StandardStemmer):
    ...
    def stem(self, word):
        ...
        # STEP 5: Undouble
        if word.endswith("enn") | word.endswith("onn")| word.endswith("ett")| word.endswith("ell") | word.endswith("eill"):
            word = word[:-1]
    ...
    def __rv_french(self, word, vowels):
        ...
        rv = ""
        if len(word) >= 2:
            if (word.startswith(("par") | word.startswith("col") |  word.startswith("tap")) or
...
class ItalianStemmer(_StandardStemmer):
    ...
    def stem(self, word):
        ...
        for suffix in self.__step1_suffixes:
            if word.endswith(suffix):
                if suffix == "amente" and r1.endswith(suffix):
                    ...
                    elif (r2.endswith(("os") | r2.endswith("ic"))):
                        word = word[:-2]
                        rv = rv[:-2]
        ...
        # STEP 3a
        if (rv.endswith("a") | rv.endswith("e") | rv.endswith("i") | rv.endswith("o") | rv.endswith("\xE0") | rv.endswith("\xE8") |
                        rv.endswith("\xEC")| rv.endswith("\xF2")):
            word = word[:-1]
            rv = rv[:-1]
...
class PortugueseStemmer(_StandardStemmer):
    ...
    def stem(self, word):
        ...
        # STEP 5
        if rv.endswith("e")| rv.endswith("\xE9") | rv.endswith("\xEA"):
            word = word[:-1]
            rv = rv[:-1]
...
class SpanishStemmer(_StandardStemmer):
    ...
    def stem(self, word):
        ...
        for suffix in self.__step0_suffixes:
            if word.endswith(suffix):
                if rv.endswith(suffix):
                    if (rv[:-len(suffix)].endswith("i\xE9ndo")|rv[:-len(suffix)].endswith("\xE1ndo")|rv[:-len(suffix)].endswith("\xE1r)"| rv[:-len(suffix)].endswith("\xE9r")|
                                                   rv[:-len(suffix)].endswith("\xEDr"))):
                    ...
                    elif (rv[:-len(suffix)].endswith("ando") | rv[:-len(suffix)].endswith("iendo")|
                                                     rv[:-len(suffix)].endswith("ar")| rv[:-len(suffix)].endswith("er")| rv[:-len(suffix)].endswith("ir")):
                        word = word[:-len(suffix)]
                        r1 = r1[:-len(suffix)]
                        r2 = r2[:-len(suffix)]
                        rv = rv[:-len(suffix)]
