...
The Natural Language Toolkit (\NLTK) was developed to give a broad
range of students access to the core knowledge and skills of \NLP.
In particular, \NLTK\ makes it feasible to run a course that covers
a substantial amount of theory and practice with an audience consisting
...
\NLTK\ has been used in over 60 university courses in 20
...
Other Python libraries are very useful in the \NLP\ context:
Numpy provides optimized support for linear algebra and sparse arrays,\footnote{\url{http://numpy.scipy.org/}}
and Pylab provides sophisticated facilities for scientific visualisation.\footnote{\url{http://matplotlib.sourceforge.net/}}
...
in computer science.  The corresponding the holy grail in linguistics is full
...
appropriate font size, avoiding the problem of output scrolling the
command out of view, and distributing a log of the instructors session
for students to go over in their own time.
