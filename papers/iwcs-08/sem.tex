...
Additionally, \texttt{Expression}s in \NLTK\ can be optionally typed.  This 
allows the user not only to display the \texttt{Expression}'s type, but to 
prevent expressions that are grammatically sound, but that cause type errors.
The parameter \texttt{type\_check=True} can be passed into the 
\texttt{LogicParser} constructor to indicate that expressions created by
the parser should be type checked.  Every typed expression has a method 
\texttt{gettype()} that returns the expression's type.  When a type error 
occurs during the construction of an \texttt{Expression}, a 
\texttt{TypeException} is raised.
\begin{Verbatim}
>>> tlp = logic.LogicParser(type_check=True)
>>> try: tlp.parse(r'\P.P(x,y)(\z.Q(z))')
... except Exception, e: print e.__class__.__name__
TypeException
>>> a = tlp.parse(r'\P.P(x)')
>>> a.gettype()
<<e,t>,t>
...
\enumsentence{\label{drt3} \texttt{DRS([j,d],[John(j), dog(d), sees(j,d)])}} 
...
\texttt{nltk.gluesemantics.glue} for Glue based on \FOL\
meaning expressions, and a parallel module
\texttt{nltk.gluesemantics.drt\_glue} for Glue based on
\DRT.
...
The \texttt{discourse} module also has the ability to handle semantic 
ambiguity and to filter out readings that are not semantically viable.
Using both Glue Semantics and \DRT, the following example processes the 
two-sentence discourse "Every dog chases a boy.  He runs".  As is shown
by the readings generated, the first sentence has two possible readings.
The second sentence contains an anaphoric pronoun.
\begin{Verbatim}
...
s1-r0: ([x],[PRO(x), run(x)])
\end{Verbatim}
When we show the readings of the two possible threads we see that 
that the first reading, where "every dog" out-scopes "a boy", is found
to be invalid because the pronoun in the second sentence cannot be 
resolved.  We are also able to filter out invalid readings using 
\texttt{filter=True}.
\begin{Verbatim}
...
\NLTK\ also includes a framework for recognizing textual entailment 
including the RTE data sets and a few simple modules to demonstrate
RTE techniques.  For example, the Logical Entailment RTE tagger is a  
semantically-oriented approach to RTE located in
\texttt{nltk.rte.logicentail}.  Based on \cite{BosRTE}, the
tagger begins by building a semantic representation in of both the text 
and the hypothesis in \DRT.  It then runs a theorem prover 
with the text as the assumption and the hypothesis as the goal in 
order to check whether the text entails the hypothesis.  If a proof is 
found, then we know that entailment holds.  The tagger is also capable 
of adding background knowledge via an interface to the WordNet 
dictionary in \texttt{nltk.wordnet} to make the entailment checking 
more robust.  
