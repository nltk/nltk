...
def text_parse(grammar, sentence, trace=2, drawtrees=False, latex=False):
	...
	parser.parse_n(sent)
	if drawtrees:
		from nltk.contrib.nltk_contrib.mit.rspeer.parser.treeview import TreeView
		TreeView(sent['TREES'])
	else:
...
Distributed under the GPL. See LICENSE.TXT for information.""" % globals()
	...
	if not batch:
		...
		if sentence == '': return
		sent=text_parse(grammar, sentence, trace, options.drawtrees, options.latex)
	else:
		...
		for line in infile.readlines():
			...
			sent=text_parse(grammar, sentence, trace, False, options.latex)
		infile.close()
        return sent
