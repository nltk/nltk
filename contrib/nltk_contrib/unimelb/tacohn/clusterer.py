...
often repeated with random initial means and the most commonly occuring
...
reparametising the axes in order of variability and discarding all bar the
...
objects. SparseArrays should be used for efficiency when required.
"""

from nltk.chktype import chktype
from nltk.probability import DictionaryProbDist
from nltk.token import Token
from nltk_contrib.unimelb.tacohn.sparsearray import SparseArray, dot, sum
    ...
    def cluster_vectorspace(self, vectors):
        ...
        for trial in range(self._repeats):
            if not self._means or trial > 1:
                self._means = random.sample(vectors, self._num_means)
...
class GroupAverageAgglomerativeClusterer(VectorSpaceClusterer):
    ...
    def cluster_vectorspace(self, vectors):
        ...
        self._dendogram = Dendogram(vectors)
    ...
    def _average_similarity(self, v1, l1, v2, l2):
        sum = v1 + v2
        length = l1 + l2
        return (dot(sum, sum) - length) / (length * (length - 1))
...
class ExpectationMaximizationClusterer(VectorSpaceClusterer):
    ...
    def __init__(self, initial_means, priors=None, covariance_matrices=None,
                       conv_threshold=1e-6, normalise=False,
                       svd_dimensions=None):
        """
    ...
    def cluster_vectorspace(self, vectors):
        ...
        while not converged:
            ...
            for j in range(self._num_clusters):
                ...
                # if the matrix is tiny (i.e. only one point)... better to
                # test if less than 'n' items in each cluster (say 3?)... bit
                # dodgy, as this may affect the other hidden variables...
                if LinearAlgebra.determinant(covariances[j]) < 1e-10:
                    covariances[j] = covariance_before
    ...
    def _gaussian(self, mean, cvm, x):
        ...
        try:
            a = 1. / Numeric.power(Numeric.power(2. * Numeric.pi, m) \
                     * LinearAlgebra.determinant(cvm), 0.5)
            dx = x - mean
            b = math.exp(-0.5 * Numeric.matrixmultiply( \
                    Numeric.matrixmultiply(dx, LinearAlgebra.inverse(cvm)), dx))
            return a * b
...
def cosine_distance(u, v):
    ...
    u.v / |u||v|.
    """
    return dot(u, v) / (math.sqrt(dot(u, u)) * math.sqrt(dot(v, v)))
...
def demo():
    ...
    clusterer.classify(token)
    print token
    # save the means for later (EM needs them)
    print

    # test the GAAC clusterer with 2 clusters
    clusterer = GroupAverageAgglomerativeClusterer(2)
    clusterer.cluster(tokens, True)
    print 'using clusterer', clusterer
    print 'clustered', str(tokens)[:60], '...'
    # show the dendogram
