#!/usr/bin/env python
# Natural Language Toolkit: Kimmo Recognizer
#
# Author: Carl de Marcken (carl at demarcken dot org)
#
# Update: Beracah Yankama
# Kimmo Recognizer updates (proper recognizer completion, generator ordering, kimmo control class, 
# console operation, + some other things I don't remember right now.
# loader for own file format, also .rul compatible with old pckimmo


# Natural Language Toolkit: GUI Interface
# v1.78
#
# Author: Beracah Yankama (beracah at mit dot edu)
# uses graphviz - dot to generate transition diagrams
# has config file
# formatted lexicon and rule files
# Author: Robert C. Berwick (berwick at ai dot mit dot edu)
# console mode
# edit windows


import Tkinter
import fnmatch, os, re, sys, types, sets, string, glob, time, md5

from nltk.chktype import chktype as _chktype 
from nltk.chktype import classeq as _classeq

from nltk_contrib.unimelb.sb.fsa import *
...
"""
Two-level morphology modeled closely on PC-KIMMO.  Can be used for
either generation or recognition.
"""
...
class KimmoGUI:
    def __init__(self, grammar, text, title='Kimmo Interface v1.78'):
        ...
        try:
            ...
            self.helpFilename = '/mit/6.863/pykimmo/pykimmo.help'
...
# 
            ...
            self.saveMenu.add_command(label='Save Results', underline=0,command = lambda filename='.results', sourceWindow = self.rules : self.writeToFilefromWindow(filename, sourceWindow,'w',0))
...
#                           command=self.generate).pack(side='right')
            ...
            # if ((os.stat('kimmo.cfg')[0] & 0400) == 0400): 
...
class KimmoControl:
    def __init__(self, lexicon_string, rule_string, lexicon_file, rule_file, debug):
    	...
    	# load lexicon file
    	if lexicon_file: 
		try: f = open(lexicon_file, 'r')
	        except IOError, e: 
	        	self.errors = 'Bad Lexicon Filename ' + lexicon_file
	        	return None	# bad filename, exit control init
	        lexicon_string = string.join(f.readlines(),"")
	        f.close()

    	# load rule file
    	if rule_file: 
		try: f = open(rule_file, 'r')
	        except IOError, e: 
	        	self.errors =  'Bad Rule Filename ' + rule_file
	        	return None	# bad filename, exit control init
    ...
    def batch(self, filename):
    	if filename:
    		try: f = open(filename, 'r')
    			...
    			self.errors = "Bad Batch File name"
    			return
    		lines = f.readlines()
    		f.close()
...
class KimmoPair:
    ...
    def _matches(self, me, terminal, subsets):
        ...
        if (me[0] == '~'):
            m = me[1:]
            if (subsets.has_key(m)):
                return not(terminal in subsets[m])
            else:
                return False
        if (subsets.has_key(me)):
...
def parse_pair_sequence(description,token_type):
    ...
    for token in tokens:
        ...
        if str == ':':
            ...
            result.append(KimmoPair(prev, str))
            prev = None
            colon = False
        else:
            if prev:
...
class KimmoDefaults:
    ...
    def __init__(self, description):
        assert _chktype(1, description, types.StringType)
        self._description = description
        self._defaults = sets.Set()
...
class KimmoFSARule:
    ...
    def sorted_pairs(self, subsets):
    	...
    	for idx, pair in enumerate(self.pairs()):	# enumerate lists all & assigns an index
    		...
    		size1 = 1
    		size2 = 1
    		if subsets.has_key(pair.input()) : size1 = len(subsets[pair.input()])
    		if subsets.has_key(pair.output()) : size2 = len(subsets[pair.output()])
    		# setsize = size1 # + size2
...
class KimmoRuleSet:
    ...
    def __init__(self, subsets, defaults, rules, null='0'):
        assert _chktype(1, subsets,  [], ())
        assert _chktype(2, defaults, [], ())
        assert _chktype(3, rules,    [], ())
        self.debug = False
        self._rules = rules
        self._pair_alphabet = sets.Set()
        self._subsets = {}
        self._null = null
        for s in subsets:
    ...
    def is_subset(self, key):
        return key[0] == '~' or self.subsets().has_key(key)
