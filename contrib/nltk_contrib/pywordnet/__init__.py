...
>>> N['dog'].senses()
('dog' in {noun: dog, domestic dog, Canis familiaris}, 'dog' in {noun: frump, dog}, 'dog' in {noun: dog}, 'dog' in {noun: cad, bounder, blackguard, dog, hound, heel}, 'dog' in {noun: pawl, detent, click, dog}, 'dog' in {noun: andiron, firedog, dog, dogiron})
>>> # Extract the first sense
>>> dog = N['dog'][0]   # aka N['dog'].senses()[0]
>>> dog
...
POINTER_TYPES = (ANTONYM, HYPERNYM, HYPONYM, ATTRIBUTE, ALSO_SEE,
...
class Word:
    ...
    ``word.senses()`` or through the index notation, ``word[n]``.
...
class Synset:
	...
	0
	"""
	return len(filter(Sense.isTagged, self.senses())) > 0
	...
	"""
	return "{" + self.pos + ": " + string.joinfields(map(lambda sense:sense.form, self.senses()), ", ") + "}"
	...
	1
	>>> N['cat'][6]
	'CAT' in {noun: computed tomography, CT, computerized axial tomography, computed axial tomography, CAT}
	"""
	senses = self.senses()
...
class Sense:
	...
	(hypernym -> {noun: canine, canid},)
	"""
	senseIndex = _index(self, self.synset.senses())
	...
	"""
	word = self.word()
	return _index(self, word.senses()) < word.taggedSenseCount
	...
	def senseIndex(sense, synset=self.synset):
	    return _index(sense, synset.senses(), testfn=lambda a,b: a.form == b.form)
...
class _IndexFile:
	...
	self.shelfname = os.path.join(WNSEARCHDIR, pos + ".pyidx")
        # causes funny exception, that's uncatchable (in python 2.3):
        #   Exception exceptions.AttributeError: "DbfilenameShelf instance
        #   has no attribute 'writeback'" in  ignored
	#try:
        #    import shelve 
